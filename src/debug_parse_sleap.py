...
print(a.shape)
a_trim,track_occupancy_trim = parse_sleap.clear_peaks_all(a,track_occupancy,plot_me=False)
...
for t_ in range(np.shape(a)[2]):
    ax.plot(a[:,0,t_],color='black',alpha=0.3)
    ax.plot(a[:,0,t_],color='blue',alpha=0.5)

ax.plot(b[:,0],alpha=0.5,color='tab:blue')

#ax.plot(simple_a,alpha=0.5)
ax.plot(c[:,0],alpha=0.8,color='red')
ax.plot(d[:,0],alpha=0.8,color='black',linestyle=':')
plt.show()
